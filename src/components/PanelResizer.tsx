...
export function PanelResizer({
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const next =
      side === "right" ? drag.startWidth + delta : drag.startWidth - delta;
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-y-0 left-1/2 -translate-x-1/2 w-[2px] transition-colors ${dragging ? "bg-blue-400" : "bg-white/[0.08] group-hover:bg-blue-400/70"}`}
      />

      {/* Grab handle (dots) */}
      <div
        className={`pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-[3px] rounded-full border px-[5px] py-[7px] transition-all ${
          dragging
