...
export class PDFExtractor implements DocumentExtractor {
  ...
  public async renderPageToImage(uint8Array: Uint8Array, pageNumber: number, scale = 2): Promise<string> {
  ...
  public async extractPageImageAsDataUrl(uint8Array: Uint8Array, pageNumber: number): Promise<string | null> {
    const pageImages = await this.extractPageImages(uint8Array, pageNumber);
    const largest = pageImages.reduce((best, img) => (img.width * img.height > (best?.width ?? 0) * (best?.height ?? 0) ? img : best), null as { width: number; height: number; channels: number; data: Uint8Array } | null);
  ...
  public async extractPageImages(uint8Array: Uint8Array, pageNumber: number): Promise<{ width: number; height: number; channels: number; data: Uint8Array }[]> {
    const result = await extractImages(uint8Array, pageNumber);
    if (!result) return [];
    const record = result as unknown as Record<string, unknown>;
    const pageKey = String(pageNumber - 1);
    const pageEntry = (record[pageKey] ?? Object.values(record)[0]) as { width?: number; height?: number; channels?: number; data?: Uint8Array } | undefined;
    if (!pageEntry?.data) return [];
    return Array.isArray(pageEntry) ? (pageEntry as unknown as { width: number; height: number; channels: number; data: Uint8Array }[]) : [pageEntry as { width: number; height: number; channels: number; data: Uint8Array }];
  }
  ...
  public async rgbToPngDataUrl(image: { width: number; height: number; channels: number; data: Uint8Array }): Promise<string> {
