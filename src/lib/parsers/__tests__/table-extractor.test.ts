...
describe("table-extractor layout path", () => {
  ...
  it("returns empty for prose without tables", () => {
    const tables = parseLayoutTables(
      "The company performed well during the year. Growth was driven by domestic demand.",
    );
...
describe("extractJsonBlock", () => {
  it("extracts JSON from a wrapped LLM reply", () => {
    const reply =
      'Here is the table:\n```json\n{"columns": ["a"], "rows": [["1"]]}\n```';
    assert.deepStrictEqual(extractJsonBlock(reply), {
      columns: ["a"],
      rows: [["1"]],
    });
  });
