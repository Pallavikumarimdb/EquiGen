 ...
 */
async function getChunkQuality(jobId: string): Promise<{ degradedCount: number; blockedFinancials: boolean }> {
  try {
    const rows = await prisma.chunkExtraction.findMany({
      where: { jobId },
      select: { extractType: true, status: true },
    });
    if (rows.length === 0) return { degradedCount: 0, blockedFinancials: false };
    const failedFinancials = rows.some((r) => r.extractType === 'financials' && r.status === 'failed');
    const degradedCount = rows.filter((r) => r.status === 'degraded' || r.status === 'failed').length;
    return { degradedCount, blockedFinancials: failedFinancials };
  } catch (err) {
    console.warn('[QueueWorker] Chunk quality check unavailable (no chunk rows yet?):', err);
    return { degradedCount: 0, blockedFinancials: false };
  }
}

async function notify(type: string, message: string, payload: Prisma.InputJsonValue | undefined, reportId?: string, jobId?: string) {
  try {
...
async function finalizeExtractionJob(
  ...
  companyName: string,
  fileName: string,
  extractedData: Awaited<ReturnType<typeof langchainAIService.extractOrResumeFinancialData>>
): Promise<void> {
  ...
  console.log(`[QueueWorker] Job ${jobId} ${degraded ? 'completed with degraded chunks' : 'successfully completed'}. Created report: ${reportId} (dataQuality: ${degraded ? 'degraded' : 'ok'})`);
}
...
export function triggerBackgroundJob(
  ...
  (async () => {
    try {
      ...
      await finalizeExtractionJob(jobId, companyName, currentJob.fileName, extractedData);
...
export function resumeBackgroundJob(
  ...
  (async () => {
    try {
      ...
      if (isBlockedResume) {
        console.log(`[QueueWorker] Resume from blocked_financials — clearing chunk rows and re-running the extraction phase.`);
        await prisma.chunkExtraction.deleteMany({ where: { jobId } });
      }

      await prisma.extractionJob.update({
        where: { id: jobId },
        data: { status: 'running', stepIndex: isBlockedResume ? 0 : 1, errorMessage: null, retryAfterSeconds: null }
      });
      ...
      await finalizeExtractionJob(jobId, job.companyName, job.fileName, extractedData);
