...
export const VISION_MODEL = process.env.GROQ_VISION_MODEL || MODEL_IDS.VISION_11B;
...
const CONFIGURED_DEFAULTS: Record<string, ModelLimitRecord> = {
  ...
  'gpt-4o-mini': { model: 'gpt-4o-mini', tpm: 200000, tpd: 2000000, source: 'configured' },
...
class ModelLimitRegistry {
  ...
  private configured(model: string): ModelLimitRecord {
    return CONFIGURED_DEFAULTS[model] ?? { model, tpm: 6000, tpd: 0, source: 'configured' };
  }

  /** Effective limit for a model, in-process and synchronous (router hot path). */
  get(model: string): ModelLimitRecord {
    const env = process.env[`GROQ_TPM_${model.toUpperCase().replace(/[^A-Z0-9_]/g, '_')}`];
    if (env) {
  ...
  async setDiscovered(record: ModelLimitRecord): Promise<void> {
    ...
    if (process.env.DATABASE_URL) {
      try {
        ...
        console.warn(`[ModelLimitRegistry] Failed persisting discovered limit for ${record.model}:`, err);
      }
    }
  }
  ...
  async hydrateFromStore(): Promise<void> {
    ...
    this.hydrating = (async () => {
      try {
        ...
        console.warn('[ModelLimitRegistry] Store hydration failed (continuing with defaults):', err);
      }
    })();
