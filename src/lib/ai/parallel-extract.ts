...
async function acquireTokens(estimatedTokens: number): Promise<void> {
  ...
  const acquire = prev.then(async () => {
    ...
    bucketTokens = Math.min(
      TPM_LIMIT,
      bucketTokens + (now - bucketRefilledAt) / REFILL_MS_PER_TOKEN,
    );
...
const ParallelChunkSchema = z.object({
  narrative: z
    .array(z.string())
    .describe(
      "Key operational facts, business details, and figures found in this section (empty if none)",
    ),
  swot: z
    .array(z.string())
    .describe(
      "SWOT strengths, weaknesses, risks, or growth drivers found in this section (empty if none)",
    ),
  financials: z
    .array(
      z.object({
        text: z
          .string()
          .describe(
            "A specific financial figure, statement line, or target with its period and unit",
          ),
        page: z
          .number()
          .describe("The exact page number the figure appears on"),
        metric: z
          .string()
          .optional()
          .describe("E.g. Revenue, EBITDA, PAT, EPS, Market Cap, or null"),
        period: z
          .string()
          .optional()
          .describe("E.g. Q1FY26, FY25, Q2FY26, or null"),
        value: z
          .number()
          .optional()
          .describe("Numeric value extracted, or null"),
...
export async function runParallelChunkExtraction(params: {
  ...
  const pages: ChunkPage[] = pageRecords
    .map((p) => ({
      ...
      isFinancial: Boolean(
        p.annotations && (p.annotations as { financials?: boolean }).financials,
      ),
    }))
  ...
  for (const page of pages) {
          ...
          pages: [
            {
              ...
              text: page.text.slice(
                ...
                (i + 1) * CHUNK_TARGET_CHARS,
              ),
            },
          ],
          charCount: Math.min(
            CHUNK_TARGET_CHARS,
            page.text.length - i * CHUNK_TARGET_CHARS,
          ),
        });
      }
      continue;
    ...
    if (
      ...
      current.charCount + page.text.length > CHUNK_TARGET_CHARS
    ) {
      chunks.push(current);
      current = { key: "", pages: [], charCount: 0 };
    }
  ...
  if (!apiKey)
    throw new Error("Groq API key not configured for parallel extraction.");
  ...
  const extractChunk = async () => {
    while (cursor < chunks.length) {
      ...
      const estimate = Math.min(
        WORKHORSE_MAX_TOKENS,
        Math.max(1500, Math.ceil(chunkText.length / 3) + 1200),
      );
      for (let attempt = 0; attempt < 2 && !res; attempt++) {
        try {
          ...
          const parsed = JSON.parse(
            cleanedText,
          ) as Partial<ParallelChunkOutput>;
          res = ParallelChunkSchema.safeParse(parsed).success
            ? (parsed as ParallelChunkOutput)
            : null;
          if (!res)
            error = new Error(
              `Output did not match extraction schema: ${cleanedText.slice(0, 300)}`,
            );
        } catch (err) {
          error = err;
      ...
      const rows: {
        extractType: string;
        content: string;
        extractionJson: Prisma.InputJsonValue | typeof Prisma.JsonNull;
        citePages: number[];
        status: string;
      }[] = [];
      ...
      if (res) {
        if (res.narrative.length > 0) {
          rows.push({
            ...
            content: res.narrative.join("\n"),
            extractionJson: res.narrative,
            citePages: chunkPages,
            status: "ok",
          });
        }
        if (res.swot.length > 0) {
          rows.push({
            extractType: "swot",
            content: res.swot.join("\n"),
            extractionJson: res.swot,
            citePages: chunkPages,
            status: "ok",
          });
        }
        ...
        const failType = chunk.pages.some((p) => p.isFinancial)
          ? "financials"
          : "narrative";
        rows.push({
          extractType: failType,
          content: "",
          extractionJson: Prisma.JsonNull,
          citePages: chunkPages,
          status: "failed",
        });
      }
      ...
      for (const row of rows) {
        const inputHash = computeSHA256(
          chunkText + row.extractType + currentSchemaVersion(),
        );
        const rowData = {
      ...
      if (res) {
        if (res.narrative.length > 0)
          lines.push(`(Narrative) ${res.narrative.join(" | ")}`);
        if (res.swot.length > 0) lines.push(`(SWOT) ${res.swot.join(" | ")}`);
        if (res.financials.length > 0)
          lines.push(
            `(Financials) ${res.financials.map((f) => `${f.text} [p.${f.page}]`).join(" | ")}`,
          );
      }
      if (lines.length === 0)
        lines.push(
          `(No structured content extracted from pages ${chunk.key}.)`,
        );
  ...
  await Promise.all(
    Array.from({ length: CHUNK_CONCURRENCY }, () => extractChunk()),
  );

  console.log(
    `[Parallel Extract] ${chunks.length} chunk(s) extracted on ${FALLBACK_GROQ_MODEL} for job ${jobId} (${degradedCount} degraded/failed).`,
  );
