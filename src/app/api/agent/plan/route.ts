...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let resolvedGoalText = (goalText || "").trim();
    ...
    if (!resolvedGoalText && resolvedCompanyName) {
      resolvedGoalText = `Initiation of coverage on ${resolvedCompanyName} (${resolvedTicker}) — 5-year DCF, peer multiples, and SEBI compliance audit`;
    }
    if (!resolvedSessionId) {
      resolvedSessionId = `session_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    }

    if (!resolvedCompanyName) {
      return NextResponse.json(
        { message: "companyName or ticker is required to generate a research plan." },
        { status: 400 }
      );
    }
